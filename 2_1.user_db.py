...
pipeline = [
    ...
    {
        "$addFields": {
            "tweet_len_description": {
                "$strLenCP": {"$ifNull": ["$user.description", ""]}
            }
        }
    },
    ...
    {
        "$addFields": {
            "user_date": {"$dateFromString": {"dateString": "$user.created_at"}}
        }
    },
    # group by user
    {
        "$group": {  # user global information
            ...
            "verified": {
                "$max": {
                    "$cond": {"if": "$user.verified", "then": 1, "else": 0}
                }
            },
            "protected": {
                "$max": {
                    "$cond": {"if": "$user.protected", "then": 1, "else": 0}
                }
            },
    ...
    {
        "$addFields": {
            "tweet_frequency": {"$divide": ["$tweet_user_count", "$user_lifetime"]}
        }
    },
    {"$addFields": {"friend_frequency": {"$divide": ["$friend_nb", "$user_lifetime"]}}},
    {
        "$addFields": {
            "aggressivity": {"$add": ["$tweet_frequency", "$friend_frequency"]}
        }
    },
    # Make the visibility attribute
    {
        "$addFields": {
            "visibility": {
                "$add": ["$mention_avg", "$hash_avg", "$url_avg", "$symbols_avg"]
            }
        }
    },
        ...
        "$addFields": {
            "ff_ratio": {
                "$cond": {
                    "if": {"$eq": ["$follower_nb", 0]},
                    "then": "$friend_nb",
                    "else": {"$divide": ["$friend_nb", "$follower_nb"]},
                }
            }
        }
    },
